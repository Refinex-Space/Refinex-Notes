...
| Run frontend tests | `npm test` | Exit 0, currently 158 Vitest assertions pass |
...
The native test command now validates compilation and runs AI provider/streaming/model-catalog, auth, workspace/file, Git engine, sync-state-machine, and search tests under `src-tauri/src/**/*.rs`. Frontend tests use Vitest and now cover editor/store behavior, Git UI/store, search helper surfaces, plus the AI context builder / stream renderer / provider selector / AI store paths under `src/**/__tests__/`. Build output remains the primary repository-level verification path.
