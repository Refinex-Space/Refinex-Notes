...
Status: ✅ Done
Evidence: `npm test -- --run src/components/git/__tests__/SyncStatus.test.tsx` 通过，覆盖同步状态文案映射、缺省时间文案、SetupPanel 初始化/clone 引导文本。
Deviations: 为了降低 SSR 测试脆弱性，组件测试优先断言 `SyncStatus` 的纯 helper 和 `SetupPanel` 的静态输出，而不是 Radix 弹层开闭行为。
