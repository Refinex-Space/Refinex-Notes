...
Status: ✅ Done
Evidence: `npm test -- --run src/components/__tests__/app-shell-utils.test.ts` 通过，新增 native search → command item 映射断言；额外 `npm run build` 通过，命令面板异步搜索 prop 已进入构建面。
Deviations: `CommandPalette` 在有查询时只显示文件搜索结果，不再并排显示静态命令组；这让 native fuzzy 结果不会再被 cmdk 的本地过滤逻辑二次裁剪。
...
| 命令面板查询时只显示文件结果 | `cmdk` 的本地过滤会与 native fuzzy 结果叠加，导致结果被意外裁剪 | 为命令组单独重写过滤逻辑 | 先保证文件搜索结果正确呈现；静态命令在空查询时仍保留原有入口 |
