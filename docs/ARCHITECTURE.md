...
- `src/components/settings/SettingsDialog.tsx` now replaces the old placeholder settings modal with a full-screen settings workspace. `GeneralSettings`, `EditorSettings`, `AIProviderConfig`, `GitSettings`, `ShortcutSettings`, and `AccountSettings` split the sections, while `src/components/ui/switch.tsx` and `src/components/ui/slider.tsx` extend the existing Radix wrapper layer for form controls.
