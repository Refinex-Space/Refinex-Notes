...
- `src-tauri/src/commands/ai.rs` now exposes `ai_chat_stream`, `ai_cancel_stream`, `ai_list_providers`, `ai_list_models`, and `ai_test_connection`, keeping provider/model truth in the native layer, loading the API key from the system keyring, and sending streamed tokens to the frontend through `tauri::ipc::Channel<String>`.
