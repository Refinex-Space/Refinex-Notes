...
- `src/components/ai/ChatPanel.tsx` now replaces the previous right-panel placeholder with a working AI conversation surface: provider/model selectors, message bubbles, Markdown rendering, streamed output cursor, stop-generation action, history clearing, current-document context chips, and `@`-triggered reference-document mentions all live inside the existing app shell.
...
- `src/components/ai/ContextBuilder.ts` now builds the MVP system prompt from the current note, cursor neighborhood, outline headings, directory tree, and open/recent files; `src/stores/aiStore.ts` owns AI panel state, stream lifecycle, provider/model loading, and cancellation semantics while reusing existing note/editor stores as context inputs.
