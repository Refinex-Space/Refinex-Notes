...
export function SetupPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 p-4">
      <section className="rounded-[1.75rem] border border-border/70 bg-[linear-gradient(180deg,rgba(248,250,252,0.96),rgba(226,232,240,0.92))] p-5 shadow-[0_16px_40px_rgba(148,163,184,0.18)] dark:bg-[linear-gradient(180deg,rgba(20,34,59,0.88),rgba(9,16,30,0.92))] dark:shadow-none">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-cyan-600/80 dark:text-cyan-200/80">
              Git Setup
            </p>
          ...
          <div className="rounded-full border border-cyan-200/50 bg-white/70 p-2 text-cyan-600 dark:border-white/10 dark:bg-white/[0.05] dark:text-cyan-100">
            <FolderGit2 className="h-5 w-5" />
          </div>
        </div>

        <div className="mt-4 flex flex-wrap items-center gap-2">
          <div className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-white/75 px-3 py-1.5 text-[11px] text-fg/90 dark:border-white/10 dark:bg-white/[0.04]">
            <GitBranch className="h-3.5 w-3.5 text-fg/75" />
            <span>{userLogin ? `GitHub 已连接：${userLogin}` : "GitHub 已登录，可直接使用 remote URL"}</span>
          </div>
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-50 px-3 py-1.5 text-[11px] text-cyan-700 dark:border-cyan-300/16 dark:bg-cyan-400/10 dark:text-cyan-50">
      ...
      <section className="rounded-3xl border border-border/70 bg-bg/70 p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-full border border-emerald-300/50 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-emerald-400/25 dark:bg-emerald-400/12 dark:text-emerald-50 dark:hover:bg-emerald-400/18"
      ...
      <section className="rounded-3xl border border-border/70 bg-bg/70 p-4">
        ...
        {errorMessage ? (
          <p className="mt-3 rounded-2xl border border-rose-300/40 bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:border-rose-300/15 dark:bg-rose-400/10 dark:text-rose-100">
            {errorMessage}
          </p>
        ) : null}
      </section>
